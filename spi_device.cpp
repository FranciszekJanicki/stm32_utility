...
void SPIDevice::transmit_bytes(std::uint8_t *const data,
...
void SPIDevice::receive_bytes(std::uint8_t *const data,
                              std::size_t const size) const noexcept {
...
void SPIDevice::read_bytes(std::uint8_t const address, std::uint8_t *const data,
                           std::size_t const size) const noexcept {
...
void SPIDevice::write_bytes(std::uint8_t const address,
                            std::uint8_t *const data,
                            std::size_t const size) const noexcept {

  if (auto write = static_cast<std::uint8_t *>(std::malloc(size + 1UL));
      write) {
    auto const command = address_to_write_command(address);
    std::memcpy(write, &command, 1UL);
    std::memcpy(write + 1UL, data, size);
...
void SPIDevice::write_byte(std::uint8_t const address,
                           std::uint8_t const data) const noexcept {
  this->write_bytes(address, std::array<std::uint8_t, 1UL>{data});
}

std::uint8_t
SPIDevice::address_to_read_command(std::uint8_t const address) noexcept {
  return address & ~(1U << (std::bit_width(address) - 1U));
}

std::uint8_t
SPIDevice::address_to_write_command(std::uint8_t const address) noexcept {
  return address | (1U << (std::bit_width(address) - 1U));
}
