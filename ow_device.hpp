...
public:
  ...
  OWDevice(TIMHandle const timer, GPIO const dev_pin,
...
private:
  ...
  static void delay_microseconds(TIMHandle const timer,
...
template <std::size_t SIZE>
std::array<std::uint8_t, SIZE>
OWDevice::read_bytes(std::uint8_t const address) const noexcept {
  auto data = std::array<std::uint8_t, SIZE>{};
