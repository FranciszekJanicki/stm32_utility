...
private:
  static std::uint8_t
  address_to_read_command(std::uint8_t const address) noexcept;
  static std::uint8_t
  address_to_write_command(std::uint8_t const address) noexcept;
...
template <std::size_t SIZE>
std::array<std::uint8_t, SIZE>
SPIDevice::read_bytes(std::uint8_t const address) const noexcept {
  auto data = std::array<std::uint8_t, SIZE>{};

  std::uint8_t command = address_to_read_command(address);

  gpio_write_pin(this->chip_select_, GPIO_PIN_RESET);
  HAL_SPI_TransmitReceive(this->spi_bus_, &command, data.data(), data.size(),
                          TIMEOUT);
  gpio_write_pin(this->chip_select_, GPIO_PIN_SET);
}
...
template <std::size_t SIZE>
void SPIDevice::write_bytes(
    std::uint8_t const address,
    std::array<std::uint8_t, SIZE> const &data) const noexcept {

  std::uint8_t command = address_to_write_command(address);
  std::array<std::uint8_t, 1UL + SIZE> _bytes{};
  std::memcpy(_bytes.data(), &command, 1UL);
  std::memcpy(_bytes.data() + 1UL, data.data(), data.size());
