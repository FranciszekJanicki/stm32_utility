...
namespace STM32_Utility {
    ...
    void I2CDevice::receive_bytes(this I2CDevice const& self,
                                  std::uint8_t* const data,
                                  std::size_t const size) noexcept
    {
    ...
    std::uint8_t I2CDevice::read_byte(this I2CDevice const& self,
    ...
    void I2CDevice::write_byte(this I2CDevice const& self,
                               std::uint8_t const address,
    ...
    {
        if (self.i2c_bus_ != nullptr) {
            for (std::uint8_t i = 0U; i < (1U << 7U); ++i) {
                if (HAL_I2C_IsDeviceReady(self.i2c_bus_, i << 1U, SCAN_RETRIES, TIMEOUT) ==
                    HAL_OK) {
    ...
    {
        if (self.i2c_bus_ != nullptr) {
            if (HAL_I2C_IsDeviceReady(self.i2c_bus_,
                                      self.dev_address_ << 1,
                                      SCAN_RETRIES,
                                      TIMEOUT) != HAL_OK) {
                std::puts("I2C ERROR\n\r");
